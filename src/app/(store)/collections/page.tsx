...
import { getPublicCollections, parseSectionConfig } from "@/lib/services/atria-banners";
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      {collections.length === 0 ? (
        ...
        <div className="mt-8 grid gap-8">
          {collections.map((c) => {
            const cfg = parseSectionConfig(c);
              ...
              <section key={c.id} className="rounded-[2rem] border border-line bg-card p-6 sm:p-8">
                <div className="flex flex-col gap-4 md:flex-row md:items-start">
                  {(cfg.imageUrl as string) ? (
                    ...
                    <img src={cfg.imageUrl as string} alt={c.title} className="h-40 w-40 shrink-0 rounded-2xl object-cover md:h-48 md:w-48" />
                  ) : null}
