...
export default async function HomePage() {
  ...
  return (
    <div>
      ...
      />
      {show("hero") ? (
        <section className="relative mx-auto max-w-7xl overflow-hidden rounded-[2.5rem]">
          <div className="relative h-[52vh] sm:h-[60vh] lg:h-[72vh]">
      ...
      {show("categories") ? (
        <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
          <div className="mb-6 flex items-end justify-between">
            <h2 className="font-display text-3xl">Shop by what you need</h2>
            ...
            {categories.slice(0, 5).map((c) => (
              ...
              >
                <p className="font-medium">
                  {c.slug === "home" && "Make home feel warmer"}
                  {c.slug === "gifts" && "Find a meaningful gift"}
                  {c.slug === "style" && "Upgrade everyday rituals"}
                  {c.slug === "travel" && "Pack for a better trip"}
                  {c.slug === "finds" && "Discover something unexpected"}
                  {!["home", "gifts", "style", "travel", "finds"].includes(c.slug) && c.name}
                </p>
