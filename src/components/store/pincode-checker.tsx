...
export function PincodeChecker() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError("Unable to quote shipping");
      setQuotes(null);
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={onSubmit} className="mt-6 rounded-2xl border border-line bg-card p-4">
      ...
      <div className="mt-2 flex gap-2">
        <input
          ...
          aria-label="Pincode"
        />
        <button className="h-11 rounded-full bg-[#161513] px-4 text-sm text-[#f6f1ea] disabled:opacity-60" type="submit" disabled={loading}>
